// This function asks the browser to get the mock data file.
// It waits while the file downloads, then turns the file into JavaScript data.
async function fetchPercentiles() {
    try {
        // Ask the browser to get the JSON file from our server.
        const response = await fetch('/mock-percentiles.json');

        // If the browser says the fetch failed, stop and show an error.
        if (!response.ok) {
            throw new Error('Failed to fetch percentiles data');
        }

        // Turn the downloaded JSON text into a real JavaScript object.
...
function createAthleteCard(athlete) {
    ...
    card.innerHTML = `
        ...
        <div class="stat-grid">
            ...
            <div class="stat-row">
                <span>Turnovers</span>
                <span class="stat-value ${getPercentileClass(latestSeason.turnovers)}">${formatPercentile(latestSeason.turnovers)}</span>
            </div>
        </div>
    `;
        ...
        <div class="detail-grid">
            <section class="detail-section">
                ...
                ${['Points', 'Assists', 'Rebounds', 'Turnovers']
                    .map((label, index) => {
                        const statKey = ['points_scored', 'assists', 'rebounds', 'turnovers'][index];
...
async function loadDashboard() {
    ...
    try {
        // Get the athletes from the mock JSON file.
